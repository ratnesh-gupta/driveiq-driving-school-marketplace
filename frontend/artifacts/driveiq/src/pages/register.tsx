...
export default function RegisterPage() {
  ...
  const handleRegister = (e: React.FormEvent) => {
    e.preventDefault();
    login(accountType);
    setLocation(accountType === "school" ? "/dashboard" : "/search");
  };
        ...
        <motion.div className="w-full max-w-sm" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            {([
              ...
              <button
                ...
                className={`p-4 rounded-xl border text-left transition-all ${accountType === item.type ? "border-primary bg-primary/5 shadow-sm" : "border-border hover:bg-muted/50"}`}
                data-testid={`button-account-type-${item.type}`}
              >
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div>
              ...
              <Input type="tel" value={phone} onChange={e => setPhone(e.target.value)} placeholder="+91 98765 43210" required data-testid="input-register-phone" />
            </div>
            <div>
              <Label>Email</Label>
